...
public class CashierMenu {
    ...
    public void show() {
        while (true) {
            ...
            int choice = readIntegerInput(); // 🔹 Gunakan metode yang aman untuk membaca input
            
            if (choice == 1) {
                processTransaction(); // 🔹 Jalankan proses transaksi
            }
            else if (choice == 0) {
...
private void processTransaction() {
    ...
    if (result.isPresent()) {
        ...
        for (int i = 0; i < numItems; i++) {
            Product product = productService.getProductById(productIds[i]); // Ambil nama produk
            if (product != null) {
